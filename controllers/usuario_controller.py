...
class RegistroController(Resource):
    def post(self):
        ...
        try:
            ...
            # Generar el hash del password para guardarla en la bd
            salt =gensalt()
            password_hashed = hashpw(bytes(data_serializada.get('password'),'utf-8'),salt).decode('utf-8')

            # reemplazamos el valor del password con el hasheo de la password
...
class LoginController(Resource):
    def post(self):
        ...
        try:
            ...
            return {'message': 'Error al hacer el login',
                    'content': error.args}
